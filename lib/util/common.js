...
function resolveLocalRef (jsonSchema, externalSchemas) {
  ...
  if (jsonSchema.oneOf || jsonSchema.anyOf || jsonSchema.allOf) {
    ...
    // Reduce oneOf/anyOf/allOf to a single object
    const reducedSchemas = schemas.reduce(function (acc, schema) {
      const json = resolveLocalRef(schema, externalSchemas)
      return { ...acc, ...json }
    }, {})

    let propName
    if (jsonSchema.anyOf) propName = 'anyOf'
    else if (jsonSchema.allOf) propName = 'allOf'
    else propName = 'oneOf'

    delete jsonSchema[propName]

    // Check if there are other properties defined other than allOf
    if (jsonSchema.properties) {
      const resolvedProperties = resolveLocalRef({ type: 'object', properties: jsonSchema.properties })
      // Merge allOf and properties
      return {
        ...reducedSchemas,
        ...resolvedProperties
      }
  ...
  // $ref in the format: #/components/schemas/<resolved definition> -> this resolves the issue with $ref inside allOf/anyOf/oneOf
